...
onMounted(async () => {
    const res = await axios.get(`${import.meta.env.VITE_API_URL_BACKEND}/api/detail/${id}`)
    data.value = res.data
})
...
async function handleSaveInfor(type) {
    if (name) {
        const res = await axios.get(`${import.meta.env.VITE_API_URL_BACKEND}/api/cart`, {
            params: {
                id: type,
                name: name,
                count: count.value,
...
<template>
    ...
    <div style="background: #F4F1EA;" class="px-4 pt-29">
        <div v-for="item in data" class="flex xl:px-45 justify-center py-15 gap-7 flex-wrap"
            ...
            <div style="width: 45%;" class="py-3">
                <div class="flex justify-between items-center">
                    ...
                    <div class="font-extrabold text-[36px]">${{ Math.floor(item.price).toLocaleString("vi-VN") }}</div>
                </div>
